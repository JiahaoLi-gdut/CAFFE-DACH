...
def _Net_forward(self, preforward_flag, blobs=None, start=None, end=None, **kwargs):
    """
    ...
    self._forward(preforward_flag, start_ind, end_ind)

    # Unpack blobs to extract
    return {out: self.blobs[out].data for out in outputs}


def _Net_backward(self, prebackward_flag, diffs=None, start=None, end=None, **kwargs):
    """
    ...
    self._backward(prebackward_flag, start_ind, end_ind)

    # Unpack diffs to extract
    return {out: self.blobs[out].diff for out in outputs}


def _Net_forward_all(self, preforward_flag, blobs=None, **kwargs):
    """
    ...
    for batch in self._batch(kwargs):
        outs = self.forward(preforward_flag, blobs=blobs, **batch)
...
def _Net_forward_backward_all(self, preforward_flag, prebackward_flag, blobs=None, diffs=None, **kwargs):
    """
    ...
    for fb, bb in izip_longest(forward_batches, backward_batches, fillvalue={}):
        batch_blobs = self.forward(preforward_flag, blobs=blobs, **fb)
        batch_diffs = self.backward(prebackward_flag, diffs=diffs, **bb)
