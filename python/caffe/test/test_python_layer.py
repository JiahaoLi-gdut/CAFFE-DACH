...
class TestPythonLayer(unittest.TestCase):
    ...
    def test_reshape(self, preforward_flag):
        s = 4
    ...
    def test_parameter(self, preforward_flag, prebackward_flag):
        net_file = parameter_net_file()
        net = caffe.Net(net_file, caffe.TRAIN)
        # Test forward and backward
        net.forward(preforward_flag)
        net.backward(prebackward_flag)
    ...
    def test_phase(self, preforward_flag):
        net_file = phase_net_file()
        for phase in caffe.TRAIN, caffe.TEST:
            net = caffe.Net(net_file, phase)
            self.assertEqual(net.forward(preforward_flag)['phase'], phase)
