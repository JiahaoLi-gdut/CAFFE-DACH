...
void DialLayer<Dtype>::LayerSetUp(
  ...
  for (int i = 0; i < this->blobs_.size(); ++i) {
    if (this->layer_param_.param_size() == i) {
      ...
      fixed_param_spec->set_lr_mult(i == 0 ? 1.f : 0.f);
    }
    else if (i != 0) {
...
template <typename Dtype>
void DialLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top,
...
template <typename Dtype>
void DialLayer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top,
