...
__global__ void EntropyLossForwardGPU(
    const int nthreads, const Dtype* prob_data, Dtype* entropy,
...
void EntropyLossLayer<Dtype>::Forward_gpu(
  ...
  softmax_layer_->Forward(softmax_bottom_vec_, softmax_top_vec_, preforward_flag);
...
template <typename Dtype>
__global__ void EntropyLossBackwardGPU(
    const int nthreads, const Dtype* prob_data, const Dtype* entropy_data,
