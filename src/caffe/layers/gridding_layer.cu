...
template <typename Dtype>
void GriddingLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top,
    const bool preforward_flag) {
  const Dtype* bias = bias_term_ ? this->blobs_[1]->gpu_data() : 0;
  if (gridding_ == "incomplete_gridding") {
    incomplete_gridding_gpu_aoxpb2y<Dtype><<<CAFFE_GET_BLOCKS(top[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
    ...
    complete_gridding_gpu_aoxpb2y<Dtype><<<CAFFE_GET_BLOCKS(top[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
      bottom[0]->shape(0), bottom[0]->shape(1), bottom[0]->shape(2), bottom[0]->shape(3),
      kernel_c_, kernel_h_, kernel_w_, stride_c_, stride_h_, stride_w_,
      this->blobs_[0]->gpu_data(), bottom[0]->gpu_data(), bias, top[0]->mutable_gpu_data());
  }
  else if (gridding_ == "shared_gridding") {
    shared_gridding_gpu_aoxpb2y<Dtype><<<CAFFE_GET_BLOCKS(top[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
      bottom[0]->shape(0), bottom[0]->shape(1), bottom[0]->shape(2), bottom[0]->shape(3),
      kernel_c_, kernel_h_, kernel_w_, stride_c_, stride_h_, stride_w_,
      this->blobs_[0]->gpu_data(), bottom[0]->gpu_data(), bias, top[0]->mutable_gpu_data());
  }
...
template <typename Dtype>
void GriddingLayer<Dtype>::Backward_gpu(
    const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    if (gridding_ == "incomplete_gridding") {
      incomplete_degridding_gpu_xoypb2a<Dtype><<<CAFFE_GET_BLOCKS(this->blobs_[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
        bottom[0]->shape(0), bottom[0]->shape(1), bottom[0]->shape(2), bottom[0]->shape(3),
        kernel_c_, kernel_h_, kernel_w_, stride_c_, stride_h_, stride_w_,
        bottom[0]->gpu_data(), top_diff, 0, this->blobs_[0]->mutable_gpu_diff());
    }
    else if (gridding_ == "complete_gridding") {
  ...
  if (propagate_down[0]) {
    if (gridding_ == "incomplete_gridding") {
      incomplete_degridding_gpu_aoxpb2y<Dtype><<<CAFFE_GET_BLOCKS(bottom[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
        bottom[0]->shape(0), bottom[0]->shape(1), bottom[0]->shape(2), bottom[0]->shape(3),
        kernel_c_, kernel_h_, kernel_w_, stride_c_, stride_h_, stride_w_,
        this->blobs_[0]->gpu_data(), top_diff, 0, bottom[0]->mutable_gpu_diff());
    }
    else if (gridding_ == "complete_gridding") {
      complete_degridding_gpu_aoxpb2y<Dtype><<<CAFFE_GET_BLOCKS(bottom[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
        bottom[0]->shape(0), bottom[0]->shape(1), bottom[0]->shape(2), bottom[0]->shape(3),
        kernel_c_, kernel_h_, kernel_w_, stride_c_, stride_h_, stride_w_,
        this->blobs_[0]->gpu_data(), top_diff, 0, bottom[0]->mutable_gpu_diff());
    }
    else if (gridding_ == "shared_gridding") {
      shared_degridding_gpu_aoxpb2y<Dtype><<<CAFFE_GET_BLOCKS(bottom[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
        bottom[0]->shape(0), bottom[0]->shape(1), bottom[0]->shape(2), bottom[0]->shape(3),
        kernel_c_, kernel_h_, kernel_w_, stride_c_, stride_h_, stride_w_,
        this->blobs_[0]->gpu_data(), top_diff, 0, bottom[0]->mutable_gpu_diff());
    }
...
INSTANTIATE_LAYER_GPU_FUNCS(GriddingLayer);
} // namespace caffe