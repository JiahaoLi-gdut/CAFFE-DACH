...
void LRNLayer<Dtype>::LayerSetUp(
  ...
  if (this->layer_param_.lrn_param().norm_region() ==
    ...
    pool_param.mutable_pooling_param()->set_pad_c(0);
    pool_param.mutable_pooling_param()->set_pad_h(pre_pad_);
    pool_param.mutable_pooling_param()->set_pad_w(pre_pad_);
    pool_param.mutable_pooling_param()->set_kernel_c(1);
    pool_param.mutable_pooling_param()->set_kernel_h(size_);
    pool_param.mutable_pooling_param()->set_kernel_w(size_);
...
void LRNLayer<Dtype>::Forward_cpu(
  ...
  switch (this->layer_param_.lrn_param().norm_region()) {
  case LRNParameter_NormRegion_ACROSS_CHANNELS:
    CrossChannelForward_cpu(bottom, top, preforward_flag);
    break;
  case LRNParameter_NormRegion_WITHIN_CHANNEL:
    WithinChannelForward(bottom, top, preforward_flag);
    break;
  default:
...
void LRNLayer<Dtype>::WithinChannelForward(
    ...
    const vector<Blob<Dtype>*>& top,
    const bool preforward_flag) {
  split_layer_->Forward(bottom, split_top_vec_, preforward_flag);
  square_layer_->Forward(square_bottom_vec_, square_top_vec_, preforward_flag);
  pool_layer_->Forward(square_top_vec_, pool_top_vec_, preforward_flag);
  power_layer_->Forward(pool_top_vec_, power_top_vec_, preforward_flag);
  product_layer_->Forward(product_bottom_vec_, top, preforward_flag);
}
...
void LRNLayer<Dtype>::Backward_cpu(
  ...
  switch (this->layer_param_.lrn_param().norm_region()) {
  case LRNParameter_NormRegion_ACROSS_CHANNELS:
    CrossChannelBackward_cpu(top, propagate_down, bottom, prebackward_flag);
    break;
  case LRNParameter_NormRegion_WITHIN_CHANNEL:
    WithinChannelBackward(top, propagate_down, bottom, prebackward_flag);
    break;
  default:
    LOG(FATAL) << "Unknown normalization region.";
  }
...
void LRNLayer<Dtype>::WithinChannelBackward(
  ...
  if (propagate_down[0]) {
    ...
    product_layer_->Backward(top, product_propagate_down, product_bottom_vec_, prebackward_flag);
    power_layer_->Backward(power_top_vec_, propagate_down, pool_top_vec_, prebackward_flag);
    pool_layer_->Backward(pool_top_vec_, propagate_down, square_top_vec_, prebackward_flag);
    square_layer_->Backward(square_top_vec_, propagate_down, square_bottom_vec_, prebackward_flag);
    split_layer_->Backward(split_top_vec_, propagate_down, bottom, prebackward_flag);
  }
}
