...
__global__ void pairwise_ssd_summation_call_gpu(
...
template <typename Dtype>
Dtype pairwise_ssd_summation_main_gpu(
    const int n_y, const int d_y,
    const int n_z, const int d_z,
    const int n_s, const int d_s,
    const int n_t, const int d_t,
  ...
  pairwise_ssd_summation_call_gpu<Dtype><<<CAFFE_GET_BLOCKS(m_t), CAFFE_CUDA_NUM_THREADS>>>(
    n_y, d_y, n_z, d_z,
    n_s, d_s, n_t, d_t,
    x_s, y, x_t, z
  );
...
template <typename Dtype>
__global__ void pairwise_mmd_summation_call_gpu(
    const int n_y, const int d_y,
    const int n_z, const int d_z,
    const int n_s, const int d_s,
    const int n_t, const int d_t,
...
template <typename Dtype>
Dtype pairwise_mmd_summation_main_gpu(
    const int n_y, const int d_y,
    const int n_z, const int d_z,
    const int n_s, const int d_s,
    const int n_t, const int d_t,
  ...
  const int u_y = m_y < m_t ? m_y : m_t;
  const int u_z = (m_y < m_t) * (m_t - m_y);
  pairwise_mmd_summation_call_gpu<Dtype><<<CAFFE_GET_BLOCKS(m_t), CAFFE_CUDA_NUM_THREADS>>>(
    n_y, d_y, n_z, d_z,
    n_s, d_s, n_t, d_t,
    n_r, d_r, n_k, g_b,
    g_m, r_p, x_s, y, x_t, z
...
template <typename Dtype>
void pairwise_mmd_derivative_gpu(
    const int d_s, const int d_t,
    const int n_k, const Dtype g_b,
...
void MMDLossLayer<Dtype>::Forward_gpu(
  ...
  const int* random_datum = random_blob_.gpu_data();
  Dtype* source_diffs = bottom[0]->mutable_gpu_diff(); // used for buffer
  Dtype* target_diffs = bottom[1]->mutable_gpu_diff(); // used for buffer
  Dtype* topper_datum = top[0]->mutable_cpu_data();
  //calculate bandwidth
  CHECK(domain_dnumb * domain_dnumb * sample_ddims >= 0)
    << "Integer value overflow!";
  const Dtype buffer_absum = pairwise_ssd_summation_main_gpu(
  ...
  *topper_datum = pairwise_mmd_summation_main_gpu(
    source_dnumb, source_ddims,
    target_dnumb, target_ddims,
    source_dnumb, source_ddims,
    target_dnumb, target_ddims,
...
void MMDLossLayer<Dtype>::Backward_gpu(
  ...
  Dtype* buffer_datum = buffer_blob_.mutable_gpu_data();
  caffe_gpu_set(source_count, Dtype(0), source_diffs);
  caffe_gpu_set(target_count, Dtype(0), target_diffs);
  const int sample_dnumb = source_dnumb < target_dnumb ? target_dnumb : source_dnumb;
