...
__global__ void mean_square_bias_call_gpu(
...
template <typename Dtype>
void mean_square_bias_main_gpu(
    const int n_s, const int d_s,
    const int n_t, const int d_t,
    const Dtype* const x_s, Dtype* const y_s,
    const Dtype* const x_t, Dtype* const y_t,
  ...
  const int m_s = n_s * d_x;
  const int m_t = n_t * d_x;
  mean_square_bias_call_gpu<Dtype><<<CAFFE_GET_BLOCKS(m_s + m_t), CAFFE_CUDA_NUM_THREADS>>>(
    n_s, d_s, n_t, d_t,
    x_s, y_s, x_t, y_t
  );
  caffe_gpu_lossness_sum(m_s, y_s, &b_s);
  caffe_gpu_lossness_sum(m_t, y_t, &b_t);
  b_x = b_s + b_t;
  b_s /= b_x;
  b_t /= b_x;
  b_s *= (n_s + n_t) / (2 * n_s);
...
template <typename Dtype>
__global__ void mean_deltas_diff_call_gpu(
    const int n_s, const int d_s,
    const int n_t, const int d_t,
...
template <typename Dtype>
void mean_deltas_diff_main_gpu(
    const int n_s, const int d_s,
    const int n_t, const int d_t,
    const Dtype r_b, const Dtype l_w,
    const Dtype* const x_s, Dtype* const y_s,
    const Dtype* const x_t, Dtype* const y_t) {
  const int d_x = d_s < d_t ? d_s : d_t;
  const int m_x = (n_s + n_t) * d_x;
  caffe_gpu_set(d_s * n_s, Dtype(0), y_s);
  caffe_gpu_set(d_t * n_t, Dtype(0), y_t);
  mean_deltas_diff_call_gpu<Dtype><<<CAFFE_GET_BLOCKS(m_x), CAFFE_CUDA_NUM_THREADS>>>(
    n_s, d_s, n_t, d_t, r_b,
    l_w, x_s, y_s, x_t, y_t
  );
}
...
template <typename Dtype>
void MSBLossLayer<Dtype>::Forward_gpu(
...
template <typename Dtype>
void MSBLossLayer<Dtype>::Backward_gpu(
    const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(MSBLossLayer);
} // namespace caffe