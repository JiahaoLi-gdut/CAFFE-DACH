...
template <typename Dtype>
void SPPLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top,
    const bool preforward_flag) {
  if (pyramid_height_ == 1) {
    pooling_layers_[0]->Forward(bottom, top, preforward_flag);
    return;
  }
  split_layer_->Forward(bottom, split_top_vec_, preforward_flag);
  for (int i = 0; i < pyramid_height_; i++) {
    pooling_layers_[i]->Forward(
        *pooling_bottom_vecs_[i], *pooling_top_vecs_[i], preforward_flag);
    flatten_layers_[i]->Forward(
        *pooling_top_vecs_[i], *flatten_top_vecs_[i], preforward_flag);
  }
  concat_layer_->Forward(concat_bottom_vec_, top, preforward_flag);
}

template <typename Dtype>
void SPPLayer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top,
  ...
  if (pyramid_height_ == 1) {
    pooling_layers_[0]->Backward(top, propagate_down, bottom, prebackward_flag);
    return;
  }
  vector<bool> concat_propagate_down(pyramid_height_, true);
  concat_layer_->Backward(top, concat_propagate_down, concat_bottom_vec_, prebackward_flag);
  for (int i = 0; i < pyramid_height_; i++) {
    flatten_layers_[i]->Backward(
        *flatten_top_vecs_[i], propagate_down, *pooling_top_vecs_[i], prebackward_flag);
    pooling_layers_[i]->Backward(
        *pooling_top_vecs_[i], propagate_down, *pooling_bottom_vecs_[i], prebackward_flag);
  }
