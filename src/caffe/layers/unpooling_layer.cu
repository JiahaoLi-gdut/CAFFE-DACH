...
//same as ave_unpooling_gpu in pooling layer.
...
//same as ave_pooling_gpu in pooling layer.
template <typename Dtype>
...
//same as max_unpooling_gpu in pooling layer.
template <typename Dtype>
...
//not same as max_pooling_gpu in pooling layer.
template <typename Dtype>
...
template <typename Dtype>
__global__ void tile_unpooling_gpu(const int n,
    const int  c_y, const int  h_y, const int  w_y,
    const int  c_k, const int  h_k, const int  w_k,
    const int  c_s, const int  h_s, const int  w_s,
    const int  c_p, const int  h_p, const int  w_p,
...
template <typename Dtype>
__global__ void tile_pooling_gpu(const int n,
    const int  c_x, const int  h_x, const int  w_x,
    const int  c_k, const int  h_k, const int  w_k,
    const int  c_s, const int  h_s, const int  w_s,
    const int  c_p, const int  h_p, const int  w_p,
...
template <typename Dtype>
void UnpoolingLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top,
    const bool preforward_flag) {
  switch (this->layer_param_.unpooling_param().unpool()) {
  case UnpoolingParameter_UnpoolMethod_AVE:
    ave_unpooling_gpu<Dtype><<<CAFFE_GET_BLOCKS(top[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
      top[0]->shape(0),
  ...
  case UnpoolingParameter_UnpoolMethod_MAX:
    max_unpooling_gpu<Dtype><<<CAFFE_GET_BLOCKS(top[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
      top[0]->shape(0),
      top[0]->shape(1), top[0]->shape(2), top[0]->shape(3),
      kernel_c_, kernel_h_, kernel_w_,
      stride_c_, stride_h_, stride_w_,
  ...
  case UnpoolingParameter_UnpoolMethod_TILE:
    tile_unpooling_gpu<Dtype><<<CAFFE_GET_BLOCKS(top[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
      top[0]->shape(0),
      top[0]->shape(1), top[0]->shape(2), top[0]->shape(3),
      kernel_c_, kernel_h_, kernel_w_,
      stride_c_, stride_h_, stride_w_,
...
template <typename Dtype>
void UnpoolingLayer<Dtype>::Backward_gpu(
    const vector<Blob<Dtype>*>& top,
  ...
  switch (this->layer_param_.unpooling_param().unpool()) {
  case UnpoolingParameter_UnpoolMethod_AVE:
    ave_pooling_gpu<Dtype><<<CAFFE_GET_BLOCKS(bottom[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
      top[0]->shape(0),
      top[0]->shape(1), top[0]->shape(2), top[0]->shape(3),
      kernel_c_, kernel_h_, kernel_w_,
      stride_c_, stride_h_, stride_w_,
  ...
  case UnpoolingParameter_UnpoolMethod_MAX:
     max_pooling_gpu<Dtype><<<CAFFE_GET_BLOCKS(bottom[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
      top[0]->shape(0),
      top[0]->shape(1), top[0]->shape(2), top[0]->shape(3),
      kernel_c_, kernel_h_, kernel_w_,
      stride_c_, stride_h_, stride_w_,
      pad_c_, pad_h_, pad_w_,
      ceil_c_, ceil_h_, ceil_w_,
      top[0]->gpu_diff(), bottom[1]->gpu_data(), bottom[0]->mutable_gpu_diff()
    ); break;
  case UnpoolingParameter_UnpoolMethod_TILE:
    tile_pooling_gpu<Dtype><<<CAFFE_GET_BLOCKS(bottom[0]->count()), CAFFE_CUDA_NUM_THREADS>>>(
      top[0]->shape(0),
      top[0]->shape(1), top[0]->shape(2), top[0]->shape(3),
      kernel_c_, kernel_h_, kernel_w_,
      stride_c_, stride_h_, stride_w_,
