...
void Solver<Dtype>::Step(int iters) {
  ...
  while (iter_ < stop_iter) {
    ...
    // preforward to caculate the mean output of every class for some layers like homomorphism layer.
    Messenger::SetIter(iter_);
    SyncMessenger::PushMessage("Any", "Any", "Solver", "Any", "SOLVER_ITER_CHANGED", 1, 0, 1, &iter_);
    if (param_.preforward_interval() && iter_ % param_.preforward_interval() == 0) {
      for (int i = 0; i < param_.preforward_iter(); ++i) {
        net_->Forward(true);
      }
    }
    // test all test nets.
    ...
    for (int i = 0; i < param_.iter_size(); ++i) {
      loss += net_->ForwardBackward(false, false);
    }
...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  // After the optimization is done, run an additional preforward
  // to caculate the mean in each class
  SyncMessenger::PushMessage("Any", "Any", "Solver", "Any", "SOLVER_ITER_CHANGED", 1, 0, 1, &iter_);
  if ((param_.preforward_interval() && iter_ % param_.preforward_interval() == 0)
    || param_.preforward_termination()) {
    for (int i = 0; i < param_.preforward_iter(); ++i) {
      net_->Forward(true);
    }
  }
...
void Solver<Dtype>::Test(const int test_net_id) {
  ...
  for (int i = 0; i < param_.test_iter(test_net_id); ++i) {
    ...
    const vector<Blob<Dtype>*>& result =
        test_net->Forward(false, &iter_loss);
