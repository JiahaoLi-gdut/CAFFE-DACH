...
TYPED_TEST(NetTest, TestLossWeight) {
  ...
  const Dtype loss = this->net_->ForwardBackward(false, false);
  ...
  for (int i = 0; i < kNumLossWeights; ++i) {
    ...
    const Dtype weighted_loss = this->net_->ForwardBackward(false, false);
...
TYPED_TEST(NetTest, TestComboLossWeight) {
  ...
  this->InitUnsharedWeightsNet(&loss_weight, &midnet_loss_weight,
                               kForceBackward);
  const Dtype loss_main_2 = this->net_->ForwardBackward(false, false);
  ...
  this->InitUnsharedWeightsNet(&loss_weight, &midnet_loss_weight,
                               kForceBackward);
  const Dtype loss_main_3 = this->net_->ForwardBackward(false, false);
  ...
  this->InitUnsharedWeightsNet(&loss_weight, &midnet_loss_weight,
                               kForceBackward);
  const Dtype loss_midnet_2 = this->net_->ForwardBackward(false, false);
  this->CopyNetBlobs(kCopyDiff, &blob_grads_loss_2);
  this->CopyNetParams(kCopyDiff, &param_grads_loss_2);
  ...
  this->InitUnsharedWeightsNet(&loss_weight, &midnet_loss_weight,
                               kForceBackward);
  const Dtype loss_midnet_3 = this->net_->ForwardBackward(false, false);
...
TYPED_TEST(NetTest, TestBackwardWithAccuracyLayer) {
  ...
  this->net_->ForwardBackward(false, false);
}
...
TYPED_TEST(NetTest, TestSharedWeightsUpdate) {
  ...
  this->net_->Forward(false);
  this->net_->Backward(false);
...
TYPED_TEST(NetTest, TestFromTo) {
  ...
  for (int i = 1; i < this->net_->layers().size(); ++i) {
    ...
    this->net_->ForwardFromTo(false, 1, 1);
    if (i < this->net_->layers().size() - 1) {
      this->net_->ForwardFrom(false, i + 1);
    }
  ...
  for (int i = 1; i < this->net_->layers().size(); ++i) {
    this->net_->BackwardTo(false, i);
    this->net_->BackwardFrom(false, i - 1);
