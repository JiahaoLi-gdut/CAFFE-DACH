...
TYPED_TEST(NeuronLayerTest, TestPReLUConsistencyReLU) {
  ...
  // Check forward
  prelu.Forward(this->blob_bottom_vec_, this->blob_top_vec_, false);
  relu.Forward(this->blob_bottom_vec_, blob_top_vec_2, false);
  ...
  prelu.Backward(this->blob_top_vec_, propagate_down, this->blob_bottom_vec_, false);
  relu.Backward(blob_top_vec_2, propagate_down, blob_bottom_vec_2, false);
...
TYPED_TEST(NeuronLayerTest, TestPReLUInPlace) {
  ...
  // Forward in-place
  ip.Forward(this->blob_bottom_vec_, this->blob_top_vec_, false);
  prelu.Forward(this->blob_top_vec_, this->blob_top_vec_, false);
  // Forward non-in-place
  ip2.Forward(blob_bottom_vec_2, blob_middle_vec_2, false);
  prelu2.Forward(blob_middle_vec_2, blob_top_vec_2, false);
  // Check numbers
  for (int s = 0; s < blob_top_2->count(); ++s) {
    EXPECT_EQ(this->blob_top_->cpu_data()[s], blob_top_2->cpu_data()[s]);
  }
  ...
  vector<bool> propagate_down;
  propagate_down.push_back(true);
  prelu.Backward(this->blob_top_vec_, propagate_down, this->blob_top_vec_, false);
  ip.Backward(this->blob_top_vec_, propagate_down, this->blob_bottom_vec_, false);
  // Backward non-in-place
  prelu2.Backward(blob_top_vec_2, propagate_down, blob_middle_vec_2, false);
  ip2.Backward(blob_middle_vec_2, propagate_down, blob_bottom_vec_2, false);
  // Check numbers
  for (int s = 0; s < blob_bottom_2->count(); ++s) {
